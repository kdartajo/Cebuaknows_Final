	...
	if(isset($_POST['post_comment'])){
		...
		if($result){
			...
			if($insert_result){
				...
				header('location: forum_page-admin.php?forum_id='. $_GET['forum_id']);
				exit;
			}
		}
	}
		...
		<div class="container">
				...
				<?php	
					...
					if($result){
						...
						echo '<h5><em>'.$row['lastname'].', '.$row['firstname'].'</em></h5>
							</div>
						';
			...
			<form action="forum_page-admin.php?forum_id=<?php echo $_GET['forum_id']; ?>" method="post" class="d-flex align-items-center gap-2 mt-3">
