<?php
	...
	if(isset($_GET['get_contentid'])){
		$get_contentid = $_GET['get_contentid'];
		...
		$stmt->bind_param('s', $get_contentid);
		$stmt->execute();
	...
	if(isset($_POST['edit_status'])){
		...
		$stmt = $conn->prepare("update content set status = ?, feedback= ? where content_id = ?");
		$stmt->bind_param('sss', $status, $feedback, $content_id);
		$result = $stmt->execute();
...
<html>
		  ...
		  <div class="modal-dialog">
			<div class="modal-content">
			  <div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">Give Feedbacks</h5>
				<!--<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>-->
				<a href="ContentList-historian.php" type="button" class="btn-close"></a>
			  </div>
			  <div class="modal-body">
				<form action="ContentList-historian.php" method="post">
				  <div class="mb-3">
					<label class="form-label">User ID</label>
					<input type="text" name="content_id" class="form-control"  value="<?php echo isset($content_id) ? $content_id : ''; ?>" readonly>
					
				  </div>
				  <div class="mb-3">
					<label class="form-label">Feedback</label>
					<textarea type="text" name="feedback" class="form-control" required></textarea>
					<div class="form-text">This message will be relayed to the Admin or the Creator only.</div>
				  </div>
				  <div class="mb-3">
					<label class="form-label">Status</label>
					<select class="form-select" aria-label="Default select example" name="status" required>
					  <option selected><?php echo isset($status) ? $status : ''; ?></option>
					  <option value="DENIED">DENIED</option>
					  <option value="ACCEPTED">ACCEPTED</option>
					
					</select>
				  </div>
				  <button type="submit" name="edit_status" class="btn btn-primary">Submit</button>
				</form>
			  </div>
			</div>
		  </div>
	...
	<div class="container">
		...
		<br>
			<h1>List of Pending Contents</h1>
		<br>
			<table id="example" class="table table-striped nowrap border shadow" style="width:100%">
				...
				<tbody>
					...
					<?php
						$sql = "select * from content where status = 'WAITING'";
							...
							while($row = mysqli_fetch_assoc($result)){
								echo '
									<tr>
										...
										<td class="truncate">'. $row['feedback'] .'</td>  
										<td>'. $row['created_at'] .'</td>
										<td>'. $row['updated_at'] .'</td>
										<td class="'.$row['status'].'">'. $row['status'] .'</td>
										<td style="display:flex;" class="grid gap-2">
											<a class="btn btn-success" style="text-decoration:none;color:white;" href="historical_site-historian.php?contentId='.$row['content_id'].'">View</a>
											<a class="btn btn-primary" style="text-decoration:none;color:white;" href="ContentList-historian.php?get_contentid='.$row['content_id'].'">Status</a>	
