<?php
	...
	if($mode_result->num_rows > 0){
		...
		while($mode_row = mysqli_fetch_assoc($mode_result)){
			...
			if($mode_row['expiration_date'] < $current_date){
		...
		<div class="p-3 mb-4 bg-light rounded-3">
			<div class="container-fluid py-5 text-center">
				...
				<p class="col-md-8 fs-4 mx-auto">
					<!-- Added mx-auto to center the text block -->Manage advertisements with ease! Here, you can review current campaigns, activate or deactivate ads. Take control of your advertisement. </p>
			</div>
		</div>
		<div class="container">
			<table id="example" class="table table-striped nowrap border shadow" style="width:100%;">
				...
				<tbody>
					<?php
						...
						$ad_stmt = $conn->prepare("select * from advertisement INNER JOIN users ON advertisement.user_id = users.user_id where users.user_id = ?");
						$ad_stmt->bind_param('s', $row['user_id']);
						$ad_stmt->execute();
						$result = $ad_stmt->get_result();
