		...
		<div class="container mt-5">
		  <!--<div class="row">
			<div class="col-md-6 mx-auto">
			  <form action="leaderboard.php" method="post" class="input-group">
				<input type="text" class="form-control" placeholder="Search quiz title" aria-label="Search" name="quiz_title">
		  ...
		  <div>
			<div class="mx-auto">
			  <?php
					...
					$quiz_title = $_GET['quiz_id'];
					
					$check_stmt = $conn->prepare("select * from quiz_leaderboard 
												INNER JOIN quiz ON quiz_leaderboard.quiz_id = quiz.quiz_id 
												INNER JOIN users ON quiz_leaderboard.user_id = users.user_id
												where quiz_leaderboard.quiz_id = ? ORDER BY score DESC");
					$check_stmt->bind_param('s', $quiz_title);
					$check_stmt->execute();
					...
					if($check_result->num_rows > 0){
						echo '
							...
							<table id="example" class="leaderboard table table-striped nowrap  border shadow" style="width:100%">
								<thead>
							...
							';
						$rank = 0;
						while($row_leader = mysqli_fetch_assoc($check_result)){
							$rank++;
							echo '<tr>';
									echo '<td>';
									...
									if ($rank == 1) {
										// Display trophy icon for first place
										echo $rank .' <i class="fas fa-trophy text-warning"></i> ' ;
										if($row_leader['user_id'] == $row['user_id']){
											echo ' <i class="fas fa-user"></i> ' ;
										}
										
									} else {
										// Display the rank for others
										echo $rank;
										if($row_leader['user_id'] == $row['user_id']){
											echo ' <i class="fas fa-user"></i> ' ;
										}
									...
									echo '</td>';
									echo'<td>'.$row_leader['user_id'].'</td>';
									echo'<td>'.$row_leader['lastname'].', '.$row_leader['firstname'].'</td>';
									echo'<td>'.$row_leader['quiz_title'].'</td>';
									echo'<td>'.$row_leader['score'].'/'.$row_leader['total_questions'].'</td>';
									echo'<td>'.$row_leader['updated_at'].'</td>
								</tr>
							';
						}
					}else{
						echo '<br>
							<br>
							<br><h2 class="text-center">No Quiz Participants yet!</h2>';
					}
					echo '</tbody>
