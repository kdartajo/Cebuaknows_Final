<?php
	...
	if(isset($_POST['post_forum'])){
		...
		if($result){
			...
			header('location: forum.php?get_content_id='. $_GET['get_content_id']);
			exit;
		}
	}
?>
...
<html lang="en">
    ...
    <body>
        ...
        <div class="container mt-5">
			<div class="p-4 bg-light rounded shadow-sm text-center mb-5">
				<h1 class="fw-bold mb-4 text-primary">Welcome to Forums</h1>
				<p class="col-lg-8 col-md-10 mx-auto fs-4 text-muted">
				...
				<button type="button" data-bs-toggle="modal" data-bs-target="#myModal" class="btn btn-lg btn-success">
			...
			<div class="row row-cols-1 row-cols-md-3 g-4">
				...
				<div class="col">
					<div class="card shadow-sm h-100">
						<div class="card-body">
							<h5 class="card-title fw-bold"><?php echo $row['forum_topic']; ?></h5>
							<p class="card-text text-muted truncate"><?php echo $row['forum_description']; ?></p>
							<div class="d-flex justify-content-between align-items-center">
								<a href="forum_page.php?forum_id=<?php echo $row['forum_id']; ?>" class="btn btn-outline-primary btn-sm">
									<i class="fas fa-newspaper"></i> View Forum
								</a>
								<small class="text-muted"><?php echo $row['lastname'] . ', ' . $row['firstname']; ?></small>
							</div>
						</div>
					</div>
				</div>
				<?php
					...
					echo "<div class='col-12 text-center text-muted'>No forums yet. Start the conversation!</div>";
				}
				?>
			</div>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						<form action="forum.php?get_content_id=<?php echo $_GET['get_content_id']; ?>" method="post">
							<div class="mb-3">
								<label for="forum_topic" class="form-label">Forum Topic</label>
								<input type="text" name="forum_topic" id="forum_topic" class="form-control" placeholder="Enter forum topic" required>
							</div>
							<div class="mb-3">
								<label for="forum_description" class="form-label">What can you say?</label>
								<textarea name="forum_description" id="forum_description" rows="3" class="form-control" placeholder="Describe the forum" required></textarea>
							</div>
							<button type="submit" name="post_forum" class="btn btn-primary w-100">Post Forum</button>
						</form>
					</div>
				</div>
			</div>
